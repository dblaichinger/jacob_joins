class UploadController < Wicked::WizardController
  ...
  steps :recipe, :country_specific_information, :user, :show_preview
  ...
  def country_specific_information
    #if session[:error]
    #  @csis = session[:error]
    #  session[:error] = nil
    #  render_wizard @csis and return
    #end

    #if cookies[:jacob_joins_csi].present?
    #  @csis = CountrySpecificInformation.find(cookies[:jacob_joins_csi])
    #else
      @csis = CountrySpecificInformation.new_set
