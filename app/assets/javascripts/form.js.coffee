activatePlaceholders = undefined
deactivatePlaceholders = undefined
hideWizardLoader = undefined
loaderMsg = undefined
showWizardLoader = undefined
window.formHashChangeHandler = ->
  Path.routes.current = null
  if window.location.hash and window.location.hash isnt "#skipstory"
...
$ ->
  ...
  $("#send").live "click", (event) ->
    ...
    showWizardLoader ->
      $("#preview_tab").stop(true, true).fadeOut 200, ->
        empty_csi_form = undefined
        empty_recipe_form = undefined
        empty_user_form = undefined
        unless window.user?
  ...
  $("#wizard").bind "tabsshow", (event, ui) ->
    ...
    $(":input", ui.panel).filter((index) ->
      $(this).attr("visibility") isnt "hidden"
    ).parent().qtip "show"

  $("#wizard").bind "tabsselect", (event, ui) ->
    actual_form = undefined
    actual_nav_link = undefined
    loadPreview = undefined
    oldTab = undefined
    oldTabIndex = undefined
    params = undefined
    url = undefined
